...
// Push works like PushFrom but collects the pushed metrics from the
// DefaultRegistry.
func Push(job string, grouping map[string]string, url string) error {
	return PushFrom(DefaultRegistry, job, grouping, url)
}

// PushAdd works like PushAddFrom but collects the pushed metrics from the
// DefaultRegistry.
func PushAdd(job string, grouping map[string]string, url string) error {
	return PushAddFrom(DefaultRegistry, job, grouping, url)
}

// PushFrom triggers a metric collection by the provided registry and pushes all
...
func PushFrom(r Registry, job string, grouping map[string]string, url string) error {
	return push(r, job, grouping, url, "PUT")
}

// PushAddFrom works like PushFrom, but only previously pushed metrics with the
// same name (and the same job and other grouping labels) will be replaced. (It
// uses HTTP method 'POST' to push to the Pushgateway.)
func PushAddFrom(r Registry, job string, grouping map[string]string, url string) error {
...
// PushCollectors works like PushFrom, but it does not collect via a
...
// PushAddCollectors works like PushAddFrom, but it does not collect via a
// registry. Instead, it collects from the provided collectors directly. It is a
// convenient way to push only a few metrics.
